...
class FormEntidade(forms.ModelForm):
    ...
    def clean_cnpj(self):
        ...
        # Quando preenchido, já deve vir formatado pelo uso da lib em javascript
    ...
    def clean_previsao_resultado(self):
        ...
        if val:
            ...
            if self.instance and self.instance.pk and self.instance.passivel_de_estender_inscricoes():
                current_tz = timezone.get_current_timezone()
                now = timezone.now().astimezone(current_tz)
                if val and self.instance.previsao_resultado and self.instance.previsao_resultado > val.date() and val.date() < now.date():
